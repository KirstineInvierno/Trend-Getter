...
@patch("boto3.client")
def test_s3connection_success_logs_and_returns_client(mock_boto_client, caplog):
...
def test_s3connection_failure_logs_and_sets_none(mock_boto_client, caplog):
...
def test_get_topics_dict_from_rds_success(caplog, test_data):
...
def test_get_topics_dict_from_rds_connection_error(caplog):
...
def test_s3_file_extractor_get_latest_file_key(fake_s3_client):
...
def test_s3_file_extractor_raises_error_on_empty(fake_s3_client):
...
def test_converter_get_latest_file_as_dicts(fake_s3_client, caplog):
...
def test_transform_messages_into_dataframe(fake_dataframe, sample_message):
